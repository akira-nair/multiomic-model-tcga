...
def optimization(x_train, y_train, x_test, y_test, trials, modality, validation_split = 0.1):
    def objective(trial):
        ...
        tf.keras.backend.clear_session()
        model_on_ae, _ = um.create_model(x_train = x_train_ae_embed, y_train = y_train, n_epochs = EPOCHS_SUPERVISED, batch_size = BATCH_SIZE_SUPERVISED, lr = LEARNING_RATE_SUPERVISED, n_hidden = N_HIDDEN, dim_hidden = HIDDEN_DIM)
        ...
        plt.clf()
        return model_on_ae.history.history['val_loss'].pop()
