...
def optimization(trials: int, attention: bool, load_study: str = None):
    ...
    if attention:
        logger.info("Using attention model.")
    mmm = mm.MultimodalModel(modalities, attention=attention)
    logger.info(f"GPU: {tf.test.gpu_device_name()}")
    ...
    if load_study is None: 
        logger.info(f"Creating a new study")
        study = optuna.create_study(study_name = "mm_hpt_study", direction = "maximize")
    else:
        logger.info(f"Using study at {load_study}")
    ...
    logger.info(f"BEST PARAMETERS {study.best_params} HAD BEST VALUE {study.best_value} \n\n Complete.")
    return study
...
def main(argv):
    ...
    if args.filepath:
        optimization(trials = args.trials, attention= args.attention,load_study=args.filepath)
    else:
        optimization(trials = args.trials, attention = args.attention)
